...
4. **Ajuste de PIS e COFINS** (`pages/pis-cofins/`) — cross-references a user-uploaded item list against a locally-registered NCM rule table to flag zero-rate PIS/COFINS items with their legal basis. Two tabs:
   ...
   - **Cadastro NCM**: registry of `{ ncm (8 digits), descricaoProduto, codigoEnquadramento, tabela, lei }`, persisted server-side via `pisCofinsStorage.js` → `/api/pis-cofins-regras`.

5. **Importação SPED** (`pages/sped/SpedImport.jsx`) — pure client-side, no backend, no persistence. User loads many NF-e XMLs + a list of 44-digit access keys (`chaves`); the tool matches each XML to a key by reading the key **from inside the XML** (`spedMatcher.js#extractChave`: `infNFe Id="NFe…"`, then `<chNFe>`, then any isolated 44-digit run) — never by filename. `conferir()` classifies into conferidos / chaves-sem-XML / XMLs-não-pedidos / XMLs-sem-chave, and the matched XMLs are bundled into a ZIP for download via `lib/zip.js` (a hand-rolled store-only ZIP writer with CRC32, no library — same "no heavy deps" ethos as the SpreadsheetML export). Original file bytes are preserved (read as `ArrayBuffer`, not re-encoded), so the exported XMLs are byte-identical to the input.
